...
import { addMember, membersSelfTest } from "./_membersCore.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Temporary, non-sensitive config probe (no auth, no PII) to diagnose the
  // service-key role. Remove once the invite flow is confirmed working.
  if (req.headers["x-members-selftest"] === "v1") {
    const probe = await membersSelfTest(env);
    res.status(probe.status).json(probe.json);
    return;
  }
